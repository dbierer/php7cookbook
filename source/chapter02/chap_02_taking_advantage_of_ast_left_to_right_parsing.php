<?php
// this illustrates left-to-right parsing
...
<form method="post">
    <!-- Language SELECT -->
    <?php echo $callback['select']('lang', $languages); ?>
    <!-- Cities SELECT -->
    <?php echo $callback['select']('city', array_keys($cities), $callback['name']); ?>
    <input type="submit" />
</form>	
<hr>
<?php echo $callback['wrap']('h1', $callback['name']($city)); ?>
<hr>
<table>
    <tr>
    <td valign="top"><?php printf('<img width=200 src="%s" />', $callback['image']($cities[$city])); ?></td>
    <td style="width: 20px;">&nbsp;</td>
    <td valign="top"><?php echo $callback['wrap']('p',  $callback['text']($lang, $city));?></td>
    </tr>
</table>
<hr>
From wikipedia.org
